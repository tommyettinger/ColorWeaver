...
//        int[] BIG_PALETTE = Coloring.SLAB256;
//        while (base.size < -1) {
        while (base.size > 63) {
...
//                        | (int) ((cgf + 0.5f) * 63) << 13;
        ...
        System.out.println("public static final byte[][] SLAB_RAMPS = new byte[][]{");
        ...
        System.out.println("public static final int[][] SLAB_RAMP_VALUES = new int[][]{");
        for (int i = 0; i < PALETTE.length; i++) {
...
//        }
        ...
        try {
            png8.writePrecisely(Gdx.files.local("Slab64.png"), pix, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            png8.writePrecisely(Gdx.files.local("Slab64_GLSL.png"), p2, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            png8.writePrecisely(Gdx.files.local("Slab256.png"), pix, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
        for (int red = 0; red < 32; red++) {
        ...
        try {
            png8.writePrecisely(Gdx.files.local("Slab256_GLSL.png"), p2, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//        PNG8 png8 = new PNG8();
        png8.palette = new PaletteReducer(PALETTE, labRoughMetric);        
        int[][] SLAB_BONUS_RAMP_VALUES = new int[256][4];
        for (int i = 1; i < PALETTE.length; i++) {
            int color = SLAB_BONUS_RAMP_VALUES[i | 128][2] = SLAB_BONUS_RAMP_VALUES[i][2] =
                    PALETTE[i];             
//            r = (color >>> 24);
//            g = (color >>> 16 & 0xFF);
//            b = (color >>> 8 & 0xFF);
            ...
            warm = warms[i];
            mild = milds[i];
            SLAB_BONUS_RAMP_VALUES[i | 64][1] = SLAB_BONUS_RAMP_VALUES[i | 64][2] =
                    SLAB_BONUS_RAMP_VALUES[i | 64][3] = color;
            SLAB_BONUS_RAMP_VALUES[i | 192][0] = SLAB_BONUS_RAMP_VALUES[i | 192][2] = color;
...
//            r = b + chromO;
            ...
            SLAB_BONUS_RAMP_VALUES[i | 192][1] = SLAB_BONUS_RAMP_VALUES[i | 128][1] =
                    SLAB_BONUS_RAMP_VALUES[i | 64][0] = SLAB_BONUS_RAMP_VALUES[i][1] =
            ...
            SLAB_BONUS_RAMP_VALUES[i | 192][3] = SLAB_BONUS_RAMP_VALUES[i | 128][3] =
                    SLAB_BONUS_RAMP_VALUES[i][3] =
                            MathUtils.clamp(r, 0, 255) << 24 |
                                    MathUtils.clamp(g, 0, 255) << 16 |
                                    MathUtils.clamp(b, 0, 255) << 8 | 0xFF;
            ...
            SLAB_BONUS_RAMP_VALUES[i | 128][0] = SLAB_BONUS_RAMP_VALUES[i][0] =
                    MathUtils.clamp(r, 0, 255) << 24 |
                            MathUtils.clamp(g, 0, 255) << 16 |
                            MathUtils.clamp(b, 0, 255) << 8 | 0xFF;
        }
        ...
        sb.append("private static final int[][] SLAB_BONUS_RAMP_VALUES = new int[][] {\n");
        for (int i = 0; i < 256; i++) {
            sb.append("{ 0x");
            StringKit.appendHex(sb, SLAB_BONUS_RAMP_VALUES[i][0]);
            StringKit.appendHex(sb.append(", 0x"), SLAB_BONUS_RAMP_VALUES[i][1]);
            StringKit.appendHex(sb.append(", 0x"), SLAB_BONUS_RAMP_VALUES[i][2]);
            StringKit.appendHex(sb.append(", 0x"), SLAB_BONUS_RAMP_VALUES[i][3]);
            sb.append(" },\n");
        ...
        PALETTE = new int[256];
        for (int i = 0; i < 64; i++) {
            System.arraycopy(SLAB_BONUS_RAMP_VALUES[i], 0, PALETTE, i << 2, 4);
        }
        sb.setLength(0);
        ...
        png8.palette = new PaletteReducer(PALETTE, labRoughMetric);
        try {
            png8.writePrecisely(Gdx.files.local("SlabBonus.png"), pix, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            png8.writePrecisely(Gdx.files.local("SlabBonus_GLSL.png"), p2, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
...
//            pix.drawPixel(i-1, 0, PALETTE[i]);
        ...
        try {
            png8.writePrecisely(Gdx.files.local("SlabBonusMagicaVoxel.png"), pix, false);
        } catch (IOException e) {
            e.printStackTrace();
        }
