 ...
 */
public class HugglyPaletteGenerator {
    public static int LIMIT = 256;
    public static double LIMIT_ROOT = 16;
    ...
    private static final A8PaletteReducer reducer = new A8PaletteReducer();
     ...
     */
    public static double vdc(int base, int s) {
        if (base <= 2) {
    ...
    private static void add(){
        ...
        double L = reverseLight(vdc(3, idx));
        double A = vdc(5, idx) - 0.5;
        double B = vdc(7, idx) - 0.5;
...
//            if(Math.abs(r - er) + Math.abs(g - eg) + Math.abs(b - eb) <= 6) return;
            if(reducer.differenceAnalyzing(e, r, g, b) <= 300.0 / LIMIT_ROOT) return;
        }
        rgba.add(rgb);
    }
...
//        Gdx.files.local("palettes/hex/headpat-"+ (LIMIT-1) +".hex").writeString(sb.toString(), false);
        Gdx.files.local("palettes/hex/huggly-"+ (LIMIT-1) +".hex").writeString(sb.toString(), false);
