...
//                    "const vec3 yBumps = vec3(1., -1., 3.);\n" +
    ...
    public static final String fragmentShaderBlue =
            "varying vec2 v_texCoords;\n" +
                    ...
                    "void main()\n" +
                    "{\n" +
                    "   float adj = (texture2D(u_blue, gl_FragCoord.xy * (1.0 / 64.0)).r - 0.5) * u_colors;" +
                    "   vec4 tgt = texture2D( u_texture, v_texCoords );\n" +
//                    "   tgt.rgb = clamp(tgt.rgb + adj, 0.0, 1.0);\n" +
    ...
    public static final String fragmentShaderBayer =
            "varying vec2 v_texCoords;\n" +
                    "varying vec4 v_color;\n" +
                    "uniform sampler2D u_texture;\n" +
                    "uniform sampler2D u_palette;\n" +
                    ...
                    "   vec4 tgt = texture2D( u_texture, v_texCoords );\n" +
//                    "   tgt.rgb = sqrt(clamp(tgt.rgb * tgt.rgb + adj, 0.0, 1.0));\n" +
                    "   tgt.rgb = clamp(sqrt(tgt.rgb) + adj, 0.0, 1.0);\n" +
                    "   tgt.rgb *= tgt.rgb;\n" +
                    "   vec4 used = texture2D(u_palette, vec2((tgt.b * b_adj + floor(tgt.r * 31.999)) * rb_adj, 1.0 - tgt.g));\n" +
                    "   gl_FragColor.rgb = v_color.rgb * used.rgb;\n" +
    ...
    public static final String fragmentShaderBayer16 =
            "varying vec2 v_texCoords;\n" +
                    "varying vec4 v_color;\n" +
                    "uniform sampler2D u_texture;\n" +
                    "uniform sampler2D u_palette;\n" +
                    ...
                    "   int x = int(mod(gl_FragCoord.x, 4.));\n" +
                    "   int y = int(mod(gl_FragCoord.y, 4.));\n" +
                    "   int xs = int(mod(gl_FragCoord.x / 4., 4.));\n" +
                    "   int ys = int(mod(gl_FragCoord.y / 4., 4.));\n" +
                    "   float adj = (bayer[y][x] + bayer[ys][xs] * 0.0625) * u_colors;\n" +
                    "   vec4 tgt = texture2D( u_texture, v_texCoords );\n" +
                    "   tgt.rgb = clamp(sqrt(tgt.rgb) + adj, 0.0, 1.0);\n" +
                    "   tgt.rgb *= tgt.rgb;\n" +
                    "   vec4 used = texture2D(u_palette, vec2((tgt.b * b_adj + floor(tgt.r * 31.999)) * rb_adj, 1.0 - tgt.g));\n" +
    ...
    public static final String fragmentShaderBaydient =
            "varying vec2 v_texCoords;\n" +
                    "varying vec4 v_color;\n" +
                    "uniform sampler2D u_texture;\n" +
                    "uniform sampler2D u_palette;\n" +
