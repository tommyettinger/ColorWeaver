...
//            sy = MathUtils.sinDeg(y * 90f);
    ...
    public static final float[] TRI_BLUE_NOISE_MULTIPLIERS_B = new float[4096];
...
//                    b1 = bdiff * 16f / (float)Math.sqrt(2048f + bdiff * bdiff);
    ...
    public Pixmap reduceGourdLUT(Pixmap pixmap) {
        ...
        pixmap.setBlending(Pixmap.Blending.None);
        int color;
        final float strength = 2.5f * (float)(ditherStrength / (populationBias * populationBias * populationBias * populationBias));
        for (int i = 0; i < 64; i++) {
