...
import colorweaver.a8.ConstantData;
        ...
        for (int py = 0; py < h; py++) {
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    int gdiff = (color >>> 16 & 255) - (used >>> 16 & 255);
                    int bdiff = (color >>> 8 & 255) - (used >>> 8 & 255);
                    r1 = (rdiff + (BlueNoise.getSeededTriangular(px, py, 0x11111) + 0.5f) * 0x1p-7f) * strength;
                    g1 = (gdiff + (BlueNoise.getSeededTriangular(px, py, 0x22222) + 0.5f) * 0x1p-7f) * strength;
                    b1 = (bdiff + (BlueNoise.getSeededTriangular(px, py, 0x33333) + 0.5f) * 0x1p-7f) * strength;
                    float modifier;
                    if(px < lineLen - 1)
                    {
                        modifier = 128 + BlueNoise.getSeededTriangular(px+1, py, 0x0F0F0);
...
//                        }
                    ...
                    if(ny < h)
                    {
                        if(px > 0)
                        {
                            modifier = 128 + BlueNoise.getSeededTriangular(px-1, ny, 0x0F0F0);
...
//                            }
                        ...
                        modifier = 128 + BlueNoise.getSeededTriangular(px, ny, 0x0F0F0);
...
//                            }
    ...
    public Pixmap reduceCoastal (Pixmap pixmap) {
        boolean hasTransparent = (paletteArray[0] == 0);
        final int lineLen = pixmap.getWidth(), h = pixmap.getHeight();
        final float[] matrix = MATRIX_MULTIPLIERS;
        float r4, r2, r1, g4, g2, g1, b4, b2, b1;
        final float s = (float) (0.2 * ditherStrength * (populationBias * populationBias * populationBias)),
                strength = s * 0.3f / (0.19f + s);
        float[] curErrorRed, nextErrorRed, curErrorGreen, nextErrorGreen, curErrorBlue, nextErrorBlue;
        if (curErrorRedFloats == null) {
            curErrorRed = (curErrorRedFloats = new FloatArray(lineLen)).items;
            nextErrorRed = (nextErrorRedFloats = new FloatArray(lineLen)).items;
        ...
        for (int py = 0; py < h; py++) {
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    if(px < lineLen - 1)
                    {
                        modifier = matrix[(px + 1 & 7) | ((py << 3) & 56)];
                        curErrorRed[px+1]   += r4 * modifier;
                        curErrorGreen[px+1] += g4 * modifier;
                        curErrorBlue[px+1]  += b4 * modifier;
                        if(px < lineLen - 2)
                        {
                            modifier = matrix[(px + 2 & 7) | ((py << 3) & 56)];
                            curErrorRed[px+2]   += r2 * modifier;
                            curErrorGreen[px+2] += g2 * modifier;
                            curErrorBlue[px+2]  += b2 * modifier;
                        }
                    ...
                    if(ny < h)
                    {
                        if(px > 0)
                        {
                            modifier = matrix[(px - 1 & 7) | ((ny << 3) & 56)];
                            nextErrorRed[px-1]   += r2 * modifier;
                            nextErrorGreen[px-1] += g2 * modifier;
                            nextErrorBlue[px-1]  += b2 * modifier;
                            if(px > 1)
                            {
                                modifier = matrix[(px - 2 & 7) | ((ny << 3) & 56)];
                                nextErrorRed[px-2]   += r1 * modifier;
                                nextErrorGreen[px-2] += g1 * modifier;
                                nextErrorBlue[px-2]  += b1 * modifier;
                            }
                        }
                        modifier = matrix[(px & 7) | ((ny << 3) & 56)];
                        nextErrorRed[px]   += r4 * modifier;
                        nextErrorGreen[px] += g4 * modifier;
                        nextErrorBlue[px]  += b4 * modifier;
                        if(px < lineLen - 1)
...
//                    if(px < lineLen - 1)
//                    {
//                        modifier = matrix[(px + 1 & 3) | ((py << 2) & 0xC)];
//                        curErrorRed[px+1]   += r4 * modifier;
//                        curErrorGreen[px+1] += g4 * modifier;
//                        curErrorBlue[px+1]  += b4 * modifier;
//                        if(px < lineLen - 2)
//                        {
//                            modifier = matrix[(px + 2 & 3) | ((py << 2) & 12)];
//                            curErrorRed[px+2]   += r2 * modifier;
//                            curErrorGreen[px+2] += g2 * modifier;
//                            curErrorBlue[px+2]  += b2 * modifier;
//                        }
...
//                        if(px > 0)
//                        {
//                            modifier = matrix[(px - 1 & 3) | ((ny << 2) & 12)];
//                            nextErrorRed[px-1]   += r2 * modifier;
//                            nextErrorGreen[px-1] += g2 * modifier;
//                            nextErrorBlue[px-1]  += b2 * modifier;
//                            if(px > 1)
//                            {
//                                modifier = matrix[(px - 2 & 3) | ((ny << 2) & 12)];
//                                nextErrorRed[px-2]   += r1 * modifier;
//                                nextErrorGreen[px-2] += g1 * modifier;
//                                nextErrorBlue[px-2]  += b1 * modifier;
//                            }
//                        }
//                        modifier = matrix[(px & 3) | ((ny << 2) & 12)];
//                        nextErrorRed[px]   += r4 * modifier;
//                        nextErrorGreen[px] += g4 * modifier;
//                        nextErrorBlue[px]  += b4 * modifier;
//                        if(px < lineLen - 1)
...
// ^ (RAW_BLUE_NOISE[(px & 63) | (y & 63) << 6] & 255) >>> 4
    ...
    public Pixmap reduceCuatro(Pixmap pixmap) {
        boolean hasTransparent = (paletteArray[0] == 0);
        final int w = pixmap.getWidth(), h = pixmap.getHeight();
        Pixmap.Blending blending = pixmap.getBlending();
        pixmap.setBlending(Pixmap.Blending.None);
        int color0, color1, color2, color3, count, used, cr, cg, cb, or, og, ob, usedIndex, placeIndex;
        final float errorMul = (float) (ditherStrength * 0.6 * populationBias * populationBias);
        for (int y = 0; y < h; y+= 2) {
            for (int x = 0; x < w; x+= 2) {
                count = 0;
                placeIndex = 0;
                color0 = pixmap.getPixel(x, y);
                if ((color0 & 0x80) == 0 && hasTransparent)
                    pixmap.drawPixel(x, y, color0 = 0);
                else {
                    ++count;
                    placeIndex |= 1;
                }
                if(x + 1 < w) {
                    color1 = pixmap.getPixel(x + 1, y);
                    if ((color1 & 0x80) == 0 && hasTransparent)
                        pixmap.drawPixel(x, y, color1 = 0);
                    else {
                        ++count;
                        placeIndex |= 2;
                    }
                ...
                if(y + 1 < h) {
                    color2 = pixmap.getPixel(x, y + 1);
                    if ((color2 & 0x80) == 0 && hasTransparent)
                        pixmap.drawPixel(x, y, color2 = 0);
                    else {
                        ++count;
                        placeIndex |= 4;
                    }
                ...
                if(x + 1 < w && y + 1 < h) {
                    color3 = pixmap.getPixel(x + 1, y + 1);
                    if ((color3 & 0x80) == 0 && hasTransparent)
                        pixmap.drawPixel(x, y, color3 = 0);
                    else {
                        ++count;
                        placeIndex |= 8;
                    }
                ...
                if(count != 0) {
                    ...
                    cr = ((color0 >>> 24)        + (color1 >>> 24)        + (color2 >>> 24)        + (color3 >>> 24)       ) / count;
                    cg = ((color0 >>> 16 & 0xFF) + (color1 >>> 16 & 0xFF) + (color2 >>> 16 & 0xFF) + (color3 >>> 16 & 0xFF)) / count;
                    cb = ((color0 >>> 8 & 0xFF)  + (color1 >>> 8 & 0xFF)  + (color2 >>> 8 & 0xFF)  + (color3 >>> 8 & 0xFF) ) / count;
                    float em = errorMul;
                    for (int i = 0; i < count; i++) {
                        int place = placeIndex & -placeIndex;
                        placeIndex ^= place;
                        int rr = Math.min(Math.max((int) (cr + er * em), 0), 255);
                        int gg = Math.min(Math.max((int) (cg + eg * em), 0), 255);
                        int bb = Math.min(Math.max((int) (cb + eb * em), 0), 255);
                        ...
                        int ax = (idx ^ idx >>> 1) & 1;
                        int ay = idx & 1;
                        pixmap.drawPixel(x + ax, y + ay, used);
                    }
                }
            }
        }
        ...
        for (int y = 0; y < h; y++) {
            for (int px = 0; px < lineLen; px++) {
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    adj = ((BlueNoise.getSeededTriangular(px, y, 0x11111) + 0.5f) * strength) + pos; int rr = MathUtils.clamp((int) (adj + ((color >>> 24)       )), 0, 255);
                    adj = ((BlueNoise.getSeededTriangular(px, y, 0x22222) + 0.5f) * strength) + pos; int gg = MathUtils.clamp((int) (adj + ((color >>> 16) & 0xFF)), 0, 255);
                    adj = ((BlueNoise.getSeededTriangular(px, y, 0x33333) + 0.5f) * strength) + pos; int bb = MathUtils.clamp((int) (adj + ((color >>> 8)  & 0xFF)), 0, 255);
                    pixmap.drawPixel(px, y, paletteArray[paletteMapping[((rr << 7) & 0x7C00)
                            | ((gg << 2) & 0x3E0)
                            | ((bb >>> 3))] & 0xFF]);
                }
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    adj = ((ConstantData.TRI_BLUE_NOISE_B[(px + 64 & 127) | (y + 64 & 127) << 6] + 0.5f) * 0.007f); // slightly inside -1 to 1 range, should be +/- 0.8925
...
//                    adj = Math.min(Math.max(adj * strength + (px + y << 4 & 16) - 8f, -24f), 24f);
                    ...
                    //                    int gg = MathUtils.clamp((int) (adj + ((color >>> 16) & 0xFF)), 0, 255);
                    //                    int bb = MathUtils.clamp((int) (adj + ((color >>> 8)  & 0xFF)), 0, 255);
                    float tbn = TRI_BLUE_NOISE_MULTIPLIERS_A[(px & 127) | (y & 127) << 7];
        ...
        for (int py = 0; py < h; py++) {
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    adj = ((BlueNoise.getSeededTriangular(px, py, 0x12345) + 0.5f) * strength); // plus or minus 255/400
        ...
        for (int py = 0; py < h; py++) {
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    er = Math.min(Math.max(((BlueNoise.getSeededTriangular(px, py, 0x33333) + 0.5f) * strength), -limit), limit) + (curErrorRed[px]);
                    eg = Math.min(Math.max(((BlueNoise.getSeededTriangular(px, py, 0x11111) + 0.5f) * strength), -limit), limit) + (curErrorGreen[px]);
                    eb = Math.min(Math.max(((BlueNoise.getSeededTriangular(px, py, 0x22222) + 0.5f) * strength), -limit), limit) + (curErrorBlue[px]);
        ...
        for (int py = 0; py < h; py++) {
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    adj = ((BlueNoise.getSeededTriangular(px, py, 0x321FE) + 0.5f) * strength); // plus or minus 255/400
        ...
        for (int py = 0; py < h; py++) {
            ...
            for (int px = 0; px < lineLen; px++) {
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    er = Math.min(Math.max(( ( (BlueNoise.getSeededTriangular(px, py, 0x33333) + 0.5f) + ((((px+1) * 0xC13FA9A902A6328FL + (py+1) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1p-16f - 0x1p+6f)) * strength) + (curErrorRed[px]), -limit), limit);
                    eg = Math.min(Math.max(( ( (BlueNoise.getSeededTriangular(px, py, 0x11111) + 0.5f) + ((((px+3) * 0xC13FA9A902A6328FL + (py-1) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1p-16f - 0x1p+6f)) * strength) + (curErrorGreen[px]), -limit), limit);
                    eb = Math.min(Math.max(( ( (BlueNoise.getSeededTriangular(px, py, 0x22222) + 0.5f) + ((((px+2) * 0xC13FA9A902A6328FL + (py-4) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1p-16f - 0x1p+6f)) * strength) + (curErrorBlue[px]), -limit), limit);
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int x = 0; x < lineLen; x++) {
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    er = Math.min(Math.max(( ( (BlueNoise.getSeededTriangular(x, y, 0xFFFFF) + 0.5f) * blueStrength + ((((x+1) * 0xC13FA9A902A6328FL + (y+1) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1.4p-24f - 0x1.4p-2f) * strength)), -limit), limit) + (curErrorRed[x]);
                    eg = Math.min(Math.max(( ( (BlueNoise.getSeededTriangular(x, y, 0x11111) + 0.5f) * blueStrength + ((((x+3) * 0xC13FA9A902A6328FL + (y-1) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1.4p-24f - 0x1.4p-2f) * strength)), -limit), limit) + (curErrorGreen[x]);
                    eb = Math.min(Math.max(( ( (BlueNoise.getSeededTriangular(x, y, 0x22222) + 0.5f) * blueStrength + ((((x+2) * 0xC13FA9A902A6328FL + (y-4) * 0x91E10DA5C79E7B1DL) >>> 41) * 0x1.4p-24f - 0x1.4p-2f) * strength)), -limit), limit) + (curErrorBlue[x]);
...
//    }
    ...
    public Pixmap reduceKnoll2 (Pixmap pixmap) {
        ...
        for (int i = 0; i < colorCount; i++) {
            paletteLightness[i] = OKLAB[0][shrink(paletteArray[i])];
        }
        final int lineLen = pixmap.getWidth(), h = pixmap.getHeight();
        Pixmap.Blending blending = pixmap.getBlending();
        pixmap.setBlending(Pixmap.Blending.None);
...
//                        int bb = MathUtils.clamp((int) (cb - eb * errorMul), 0, 255);
                    ...
                    sort16(candidates);
//                    pixmap.drawPixel(px, y, candidates[thresholdMatrix[((px & 3) | (y & 3) << 2)]]);
                    pixmap.drawPixel(px, y, candidates[BlueNoise.getSeededTriangular(px, y, 0x12345) + 128 >>> 4]);
                }
            }
        }
        pixmap.setBlending(blending);
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    switch ((px << 1 & 2) | (y & 1)){
                        case 0:
                            ...
                            break;
                        case 1:
                            noise += (BlueNoise.getSeededTriangular(px, y, 0x13579) + 0.5f) * 0x1p-5f;
                            noise += ((y * 0x91E10DA5C79E7B1DL - px * 0xC13FA9A902A6328FL) >> 41) * 0x1p-20f;
                            break;
                        case 2:
                            noise += (BlueNoise.getSeededTriangular(px, y, 0x13579) + 0.5f) * 0x1p-6f;
                ...
                if (hasTransparent && (color & 0x80) == 0) /* if this pixel is less than 50% opaque, draw a pure transparent pixel. */
                    ...
                    switch ((px << 1 & 2) | (y & 1)){
                        case 0:
                            ...
                            eg += (BlueNoise.getSeededTriangular(px, y, 0x11111) + 0.5f) * 0x1p-5f;
                            eg += ((px * -0xC13FA9A902A6328FL + y * 0x91E10DA5C79E7B1DL) >> 41) * 0x1p-20f;
                            eb += (BlueNoise.getSeededTriangular(px, y, 0x22222) + 0.5f) * 0x1p-6f;
                            eb += ((y * 0xC13FA9A902A6328FL + px * -0x91E10DA5C79E7B1DL) >> 41) * 0x1.8p-20f;
                            break;
                        case 1:
                            er += (BlueNoise.getSeededTriangular(px, y, 0x00000) + 0.5f) * 0x1p-5f;
                            er += ((px * -0xC13FA9A902A6328FL + y * 0x91E10DA5C79E7B1DL) >> 41) * 0x1p-20f;
                            eg += (BlueNoise.getSeededTriangular(px, y, 0x11111) + 0.5f) * 0x1p-6f;
                            ...
                            break;
                        case 2:
                            er += (BlueNoise.getSeededTriangular(px, y, 0x00000) + 0.5f) * 0x1p-6f;
                        ...
                        default: // case 3:
                            ...
                            eg += ((px ^ y) % 9 - 4);
                            eg += ((px * 0xC13FA9A902A6328FL + y * 0x91E10DA5C79E7B1DL) >> 41) * 0x1p-20f;
                            eb += (BlueNoise.getSeededTriangular(px, y, 0x22222) + 0.5f) * 0x1p-5f;
                            eb += ((px * -0xC13FA9A902A6328FL + y * 0x91E10DA5C79E7B1DL) >> 41) * 0x1p-20f;
                            break;
                    }
...
//                    b1 = bdiff * 16f / (float)Math.sqrt(2048f + bdiff * bdiff);
    ...
    private static double signPreservingSquare(double n) {
        return n * Math.abs(n);
    }
    /**
     * An intentionally low-fidelity dither, meant for pixel art.
