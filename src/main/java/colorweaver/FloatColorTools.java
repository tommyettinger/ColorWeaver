...
public class FloatColorTools {
    ...
    {
        return NumberUtils.floatToIntBits(encoded) & 0x000000ff;
    }
    ...
    {
        return (NumberUtils.floatToIntBits(encoded) & 0x0000ff00) >>> 8;
    }
    ...
    {
        return (NumberUtils.floatToIntBits(encoded) & 0x00ff0000) >>> 16;
    }
...
B = t - Co;
     ...
     * @param packed a packed float color, as produced by {@link Color#toFloatBits()}
     ...
     * SquidLayers. The float is likely to be different than the result of {@link Color#toFloatBits()} unless hue,
     ...
     * SparseLayers or SquidLayers. The float is likely to be different than the result of {@link Color#toFloatBits()} unless
     ...
     * {@link Color#toFloatBits()} unless hue saturation, value, and opacity are all 0. This won't allocate any objects.
     * <br>
     * The parameters this takes all specify additive changes for a color component, clamping the final values so they
     * can't go above 1 or below 0, with an exception for hue, which can rotate around if lower or higher hues would be
     * used. As an example, if you give this 0.4f for saturation, and the current color has saturation 0.7f, then the
     ...
     * as from {@link Color#toFloatBits()} or {@link #floatGet(float, float, float, float)}, and change can be between 0f
     ...
     * start as-is) and 1f (return white), start should be a packed color, as from {@link Color#toFloatBits()} or
     ...
     * start as-is) and 1f (return black), start should be a packed color, as from {@link Color#toFloatBits()} or
     * {@link #floatGet(float, float, float, float)}. This is a good way to reduce allocations of temporary Colors, and
     * is a little more efficient and clear than using {@link #lerpFloatColors(float, float, float)} to lerp towards
